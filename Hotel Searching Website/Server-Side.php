<?php
	...
	// include Captcha class from another php file.
	include_once ("classes/Captcha.class.php");
	
	// open session
	session_start();

	// if get captcha from server side.
    if (isset($_GET['act'])){
    	header("Content-Type: image/png");
		$captcha = new Captcha(144,32);	
		$captcha->generate();
    }
    // if user want to add new account.
        ...
        $Tempcaptcha = $_POST['captcha'] ? $_POST['captcha'] : '';

        // check captcha whether is empty
        if (empty($Tempcaptcha)) {
        	echo("The Captcha must be fill in.");
        	exit();
        } 

        if (!Captcha::checkCaptcha($Tempcaptcha)) {
	...
	else if(isset($_POST['modify']))
	{
		...
		$level = $_POST['level'];
		// get user name.
		$usernName = isset($_POST['name']) ? $_POST['name']: "";
